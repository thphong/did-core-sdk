...
/** In-memory cache of resolved DID Documents */
...
/** Quick check */
export function isIotaDid(did: string): boolean {
    return typeof did === "string" && did.startsWith("did:iota:");
}

/** Build resolver URL for a given did:iota using a base like ".../api/v1" */
export function didIotaToUrl(did: string, resolverBaseUrl: string): string {
 ...
 * or per-call via resolveDidIOTA(..., { resolverBaseUrl: "..." }).
 */
export const didIOTA: DidMethod = {
    ...
    async resolve(did: string): Promise<DidDocument> {
        ...
        const { resolverBaseUrl, fetchFn, timeoutMs, cacheTtlMs } = defaults;
...
/**
 * Resolve did:iota with explicit options (overrides module defaults).
 * Useful when you want per-call control (e.g., choose devnet vs mainnet).
 */
export async function resolveDidIOTA(
    did: string,
    options: DidIOTAResolveOptions = {}
): Promise<DidDocument> {
    if (!isIotaDid(did)) throw new Error("Not a did:iota DID");
 ...
 */
export function createIotaDocTemplateFromJwk(publicKeyJwk: JsonWebKey): DidDocument {
    ...
    const didPlaceholder = "did:iota:UNPUBLISHED"; // replace with actual DID after publishing
